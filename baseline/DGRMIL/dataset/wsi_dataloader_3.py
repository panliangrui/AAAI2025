...
class C16DatasetV3(Dataset):
    def __init__(self, args,split_path,train):
        super().__init__()
        self.args = args
        self.train=train
        ...
        for i, (name, la) in enumerate(zip(slide_names, pt_label)):
            h5_path = os.path.join(args.data, name + ".h5")
            if os.path.exists(h5_path):
                with h5py.File(h5_path, 'r') as file:
                    bag = torch.from_numpy(file['features'][:]).to(self.device)
                    self.featureList.append(bag)
                label = torch.zeros(self.args.num_classes)
    ...
    def __getitem__(self, idx):
        ...
        label =  self.bag_all[idx]  
        feats =  self.featureList[idx]
        if self.train == 'train' and self.args.dropout_patch > 0.0:
            #print('drop')
            feats = dropout_patches(feats, self.args.dropout_patch)
...
def dropout_patches(feats, p):
    ...
    pad_feats = sampled_feats[pad_idx]
    sampled_feats = torch.cat((sampled_feats, pad_feats))
