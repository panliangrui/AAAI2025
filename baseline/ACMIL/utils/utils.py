...
def save_gan_model(conf,epoch,projection,
                        causal_encoder,trvial_encoder,
                        true_classifier,fake_classifier,causal_classifier,
                   is_best=False, is_last=False,seed=0):
    to_save = {
        'projection': projection.state_dict(),
        'causal_encoder':causal_encoder.state_dict(),
        'trvial_encoder':trvial_encoder.state_dict(),
        'true_classifier':true_classifier.state_dict(),
        'fake_classifier':fake_classifier.state_dict(),
         'causal_classifier':causal_classifier.state_dict(),
        'epoch': epoch,
        'config': conf,
    }
    ...
    if is_best:
        checkpoint_path = os.path.join(conf.ckpt_dir, 'seed{}_checkpoint-best.pth'.format(seed))

    if is_last:
        checkpoint_path = os.path.join(conf.ckpt_dir, 'seed{}_checkpoint-last.pth'.format(seed))

    torch.save(to_save, checkpoint_path)
def save_model(conf, epoch, model, optimizer, is_best=False, is_last=False,seed=0):
    to_save = {
