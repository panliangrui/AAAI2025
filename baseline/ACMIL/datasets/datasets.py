...
def load_data(data,split_path):
    # 从csv文件中提取某一个sheet表中的file_name列和label列数据
    file_labels_df = pd.read_csv(split_path)
    slide_names = file_labels_df['path'].tolist()
    pt_label = file_labels_df['STAS'].tolist()

    x={}
    for i,(name,la) in enumerate(zip(slide_names,pt_label)):
        location="cuda" if i<500 else "cpu"

        h5_path = os.path.join(data, name + ".h5")
        if os.path.exists(h5_path):
            with h5py.File(h5_path, 'r') as file:
                instances = torch.from_numpy(file['features'][:]).to(location)
            x[name] = {'input': instances, 'label': la}
    return x,slide_names


def split_dataset_camelyon(conf):
    train_split, train_names=load_data(conf.data_path,conf.train_excel_path)
    val_split, val_names=load_data(conf.data_path,conf.val_excel_path)
    test_split, test_names=load_data(conf.data_path,conf.test_excel_path)
...
def build_HDF5_feat_dataset(conf):
    train_split, train_names, val_split, val_names, test_split, test_names = split_dataset_camelyon(conf)
