...
class BClassifier(nn.Module):
    def __init__(self, input_size, output_class, dropout_v=0.0, nonlinear=True, passing_v=False, confounder_path=False): # K, L, N
        ...
        if confounder_path:
            ...
            conf_tensor=torch.from_numpy(np.load(confounder_path)).view(-1,input_size).float()
    ...
    def forward(self, feats, c): # N x K, N x C
        ...
        if self.confounder_path:
            if 'agnostic' in self.confounder_path[0]:
                ...
                conf_k = conf_k.view(self.confounder_feat.shape[0],bag_q.shape[0],conf_k.shape[1]) # C*k x K ---k x C x Q

                # conf_k = conf_k.view( bag_q.shape[0],self.confounder_feat.shape[0], self.confounder_feat.shape[1]) # C*k x K ---k x C x Q
                ...
                confounder_feat=self.confounder_feat.view(self.confounder_feat.shape[0],1,self.confounder_feat.shape[1])

                conf_feats = torch.einsum(' kcb ,kcq-> bcq ', A, confounder_feat)
