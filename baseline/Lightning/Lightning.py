...
class Lightning():
    ...
    def inference(self, bag_label, bag_feats, model, optimzier=None, current_result=None, train=False):
        if train:
            if self.args.model_name=="dsmil":
                ins_probs, probs = model(bag_feats)
                max_prediction, _ = torch.max(ins_probs, 0)

                loss = 0.5*self.loss_fn(probs.view(1, -1), bag_label)+0.5*self.loss_fn(max_prediction.view(1, -1), bag_label)

            else:
                probs = model(bag_feats)
                loss = self.loss_fn(probs, bag_label)
            ...
            # acc = self.melg.Accuracy(probs, bag_label)
            # current_result["acc"] += acc
        else:
            with torch.no_grad():
                if self.args.model_name=="dsmil":
                    ins_probs, probs = model(bag_feats)
                    max_prediction, _ = torch.max(ins_probs, 0)
                    probs = 0.5*torch.sigmoid(max_prediction)+0.5*torch.sigmoid(probs)
                else:
                    probs = model(bag_feats)
    ...
    def train(self,):
        ...
        for epoch in range(self.args.start_epoch, self.args.num_epochs):
            ...
            for bag_label, bag_feats in tqdm(train_dataloder, desc="{} {} Seed {} Training {}".format(self.args.model_name,
                ...
                self.inference(bag_label, bag_feats.squeeze(0), model, optimizer, current_result, train=True)
    ...
    def val(self,model,val_dataloader):
        ...
        with torch.no_grad():
            for bag_label, bag_feats in val_dataloader:
                ...
                probs = self.inference(bag_label, bag_feats.squeeze(0), model,None,None,False)

                val_labels.extend(bag_label.cpu().numpy())
                val_predictions.extend(probs.cpu().numpy())

        test_labels = np.array(val_labels)
        test_predictions = np.array(val_predictions)
        current_result=self.melg.get_reslut(test_predictions, test_labels)
